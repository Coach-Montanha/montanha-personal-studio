...
//#region node_modules/.nitro/vite/services/ssr/assets/pt-student-access.functions-B3uBRFac.js
/**
* Admin cria (ou redefine) a conta de login para um aluno de Personal Trainer.
* Retorna a senha temporária gerada.
*/
var createPTStudentAccount_createServerFn_handler = createServerRpc({
	id: "a6d2248b5dfb3cafdb231315238bcf5277959b56cb4315797f33bb9e78cbc53c",
	name: "createPTStudentAccount",
	filename: "src/lib/pt-student-access.functions.ts"
}, (opts) => createPTStudentAccount.__executeServer(opts));
var createPTStudentAccount = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => {
	...
	return input;
}).handler(createPTStudentAccount_createServerFn_handler, async ({ data, context }) => {
	...
	const { data: student, error: sErr } = await supabase.from("pt_students").select("id, user_id, account_user_id, name").eq("id", data.studentId).maybeSingle();
	...
	if (student.account_user_id) {
		...
		const { error: sUpdErr } = await supabaseAdmin.from("pt_students").update({
	...
	const { error: linkErr } = await supabaseAdmin.from("pt_students").update({
...
//#endregion
export { createPTStudentAccount_createServerFn_handler };
