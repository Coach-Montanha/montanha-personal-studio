...
//#region node_modules/.nitro/vite/services/ssr/assets/student-access.functions-DgQTv29x.js
/**
* Admin cria a conta de login para um aluno já cadastrado.
* Retorna a senha temporária gerada.
*/
var createStudentAccount_createServerFn_handler = createServerRpc({
	id: "71728a3b2f750513f96be6b8fb8ebca8e18d608e9ec31b3f6bc953a89f523242",
	name: "createStudentAccount",
	filename: "src/lib/student-access.functions.ts"
...
var createStudentAccount = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => {
	...
	return input;
}).handler(createStudentAccount_createServerFn_handler, async ({ data, context }) => {
	...
	const { data: student, error: sErr } = await supabase.from("students").select("id, user_id, account_user_id, name").eq("id", data.studentId).maybeSingle();
	...
	if (student.account_user_id) {
		...
		for (let attempt = 0; attempt < 5; attempt++) {
			const { error: uErr } = await supabaseAdmin.auth.admin.updateUserById(student.account_user_id, {
		...
		const { error: sUpdErr } = await supabaseAdmin.from("students").update({
	...
	let existingAuthUserId = null;
	{
		...
		for (let attempt = 0; attempt < 5; attempt++) {
			...
			if (res.error && /already|registered|exists|duplicate/i.test(res.error.message)) {
				let foundId = (await supabaseAdmin.auth.admin.getUserByEmail?.(data.email))?.data?.user?.id ?? null;
				if (!foundId) for (let page = 1; page <= 20 && !foundId; page++) {
	...
	const { error: linkErr } = await supabaseAdmin.from("students").update({
...
//#endregion
export { createStudentAccount_createServerFn_handler };
